...
relevant_line_pattern = column_search_pattern = Regexp.new(
  TextMate::UI.request_string(:title => 'Align by Pattern', :prompt => 'Please enter a pattern to align text:')
)
...
block_top.upto(block_bottom) do |number|
   if lines[number-1] =~ relevant_line_pattern then
      ...
      before = $`
      after = lines[number-1][before.length..-1]
      lines[number-1] = [before.ljust(best_column), after].join
   end
end
